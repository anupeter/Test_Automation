...
feature 'Health Insurance Buy Quote Test Suite', :uae do
  ...
  scenario 'Verify user can buy health insurance for family' do
    ...
    insuranceFor = 'No'
    salaryOver4k= 'Yes'
    @app.mobile_health_landing.click_on("get_quotes", true)
    ...
    first_price =  @app.mobile_health_quote_list.price.text.gsub(/[^0-9]/, '').to_i
    @app.mobile_health_quote_list.price.click
    total_price = @app.mobile_health_insurance_checkout.total_price.text.gsub(/[^0-9]/, '').to_i
    expect(total_price).to eq(first_price)
    @app.mobile_health_insurance_checkout.secure_checkout
    @app.mobile_health_insurance_payment.make_payment("#{@card_number}", "#{@expiry_date}", "#{@cvv}", "#{@card_holder}")
    @app.mobile_health_insurance_thank_you.verify_payment_received
    policy_number = @app.mobile_health_insurance_thank_you.reference_number.text.gsub(/[^0-9]/, '').split(//).last(5).join.to_i
    set_host ("broker")
    @app.mobile_broker_login.load
    @app.mobile_broker_login.login("cansu@yallacompare.com", "111111")
    set_host ("broker")
    @app.mobile_broker_health_insurance_policy_details.load type: "health", policy_number: "#{policy_number}"
    @app.mobile_broker_health_insurance_policy_details.verify_policy_status("PAID")
  end
  ...
  scenario 'Verify user can buy health insurance for yourself' do
    ...
    nationality = 'Turkish'
    salaryOver4k= 'Yes'
    @app.mobile_health_landing.choose_insurance_option("Yourself")
    @app.mobile_health_landing.click_on("get_quotes", true)
    @app.mobile_health_insurance_details.choose_member_details("#{nationality}", "#{gender}", "#{cityId}", "#{@name}",
    ...
    @app.mobile_health_declaration_details.choose_option_from_radio
    @app.mobile_health_declaration_details.get_button.click
    @app.mobile_health_quote_list.verify_quotes_details("#{@name}", "#{@email}", "#{@mobile}", "#{cityId}", "")
    page.scroll_to(@app.mobile_health_quote_list.all_good_button)
    @app.mobile_health_quote_list.all_good_button.click
    first_price =  @app.mobile_health_quote_list.price.text.gsub(/[^0-9]/, '').to_i
    @app.mobile_health_quote_list.price.click
  ...
  scenario 'Verify user can buy health insurance for maid/driver' do
    ...
    nationality = 'Turkish'
    salaryOver4k= 'Yes'
    @app.mobile_health_landing.choose_insurance_option("Your maid / driver")
    @app.mobile_health_landing.click_on("get_quotes", true)
    @app.mobile_health_insurance_details.choose_member_details("#{nationality}", "#{gender}", "#{cityId}", "#{@name}",
  ...
  scenario 'Verify user can buy health insurance for family with non-registered email address' do
    gender = 'Male'
    cityId = 'Dubai'
    nationality = 'Turkish'
    relationship = 'Child'
  ...
  scenario 'Verify user can buy health insurance for yourself with non-registered email address' do
    gender = 'Male'
    cityId = 'Dubai'
    nationality = 'Turkish'
    salaryOver4k= 'Yes'
  ...
  scenario 'Verify user can buy health insurance for maid/driver with non-registered email address' do
    gender = 'Male'
    cityId = 'Dubai'
    nationality = 'Turkish'
    salaryOver4k= 'Yes'
