...
feature 'Car PA Test Suite', :skip_qat, :uae do
  ...
  scenario 'TC-PA-001 - To buy PA quote and auto issuance' do
    ...
    Agency_Years.size.times do |i|
      ...
      actual_start_date=@app.broker_car_insurance_policy_details.actual_start_date.text
      reverse_date=Date.parse("#{actual_start_date}").strftime('%Y-%m-%d')
      merchant_ref=@app.broker_car_insurance_policy_details.merchant_ref.text
      page.scroll_to(find(:xpath,"//span[text()='Additional Addon']"))
      @app.broker_pa_insurance_policy_details.pa_link.click
      sleep 5

      expect(page).to have_xpath("//a[text()='Source car quote: #{policy_number} ']")
      sleep 60
      sleep 60
      sleep 60
      visit current_path
      @app.broker_pa_insurance_policy_details.verify_pa_policy_status("ISSUED")
      expect(page).to have_xpath("//tr[@class='odd docRow']/td[text()='Policy']")
      @app.broker_pa_insurance_policy_details.pa_verification("Salama","#{reverse_date}","#{merchant_ref}")
  ...
  scenario 'TC-PA-002 - To issue existing PA' do
    set_host ("broker")
    @app.desktop_broker_login.load
    @app.desktop_broker_login.login("anu.peter@yallacompare.com", "BrookerYC@2020")
    @app.broker_pa_insurance_policy_details.load
    @app.broker_pa_insurance_policy_details.pa_search_box.send_keys("#{$test_data['OLD_PA_EMAIL']}")
    @app.broker_pa_insurance_policy_details.pa_search_button.click
    sleep 1
    @app.broker_pa_insurance_policy_details.existing_pa.click
    sleep 2
    @app.broker_pa_insurance_policy_details.source_quote.click
    @app.broker_car_insurance_policy_details.change_policy_status("RECEIVED")
    @app.broker_car_insurance_policy_details.verify_policy_status("RECEIVED")
