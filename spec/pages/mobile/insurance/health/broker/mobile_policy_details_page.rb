...
class MobileBrokerHealthInsurancePolicyDetails < BasePage
  ...
  element :yes_button, :xpath, "//input[@class='btn btn-success btn-lg pull-right' and @value='Yes']"
  ...
  element :payment_method, :css, "[id='healthPaymentMethodId']"
  element :payment_option, :xpath, "//select[@class='form-control']//option[@value='1']"
  ...
  def issue_policy(policy_number, policy_price, note, start_date)
    ...
    click_on(payment_method, false)
    click_on(payment_option, false)
    policy_start_date.send_keys(start_date)
    click_on(auth_amount, false)
